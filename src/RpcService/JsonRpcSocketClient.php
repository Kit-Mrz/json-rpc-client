...
use Mrzkit\JsonRpcClient\Contracts\RpcServiceContract;
use Mrzkit\JsonRpcClient\Exceptions\RpcServiceException;
use Mrzkit\JsonRpcClient\Packer\JsonPacker;
...
class JsonRpcSocketClient implements RpcServiceContract
{
    ...
    public function getSocketClient()
    {
        if (is_null($this->socketClient)) {
            throw new RpcServiceException('Socket Client is NULL.');
        }
    ...
    protected function assertSocket() : self
    {
        ...
        if ($errorCode > 0) {
            throw new RpcServiceException($errorCode, socket_strerror($errorCode));
        }
    ...
    public function write($jsonEncodeString)
    {
    ...
    public function waitCall(array $params)
    {
        ...
        $content = $this->getJsonPacker()->setData($content)->unpack();
